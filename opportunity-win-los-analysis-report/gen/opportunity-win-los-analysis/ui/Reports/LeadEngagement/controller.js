angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', 'messageHub', 'entityApi', 'Extensions', function ($scope, messageHub, entityApi, Extensions) {
		...
		//-----------------Custom Actions-------------------//
		Extensions.get('dialogWindow', 'opportunity-win-los-analysis-report-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Reports" && e.view === "LeadEngagement" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.filter(e => e.perspective === "Reports" && e.view === "LeadEngagement" && e.type === "entity");
		});
		...
		$scope.openDetails = function (entity) {
			$scope.selectedEntity = entity;
			messageHub.showDialogWindow("LeadEngagement-Report-details", {
				action: "select",
				entity: entity,
			});
		};
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("LeadEngagement-Report-filter", {
